...
void readFile(vector<Job>&);
void printFileVector (const vector<Job>&);
void printOutput (const vector<Job>&);

bool sortArrival(Job &j1, Job &j2) { return j1.getArrivalTime() < j2.getArrivalTime(); }
bool sortDuration(Job &j1, Job &j2) { return j1.getJobDuration() < j2.getJobDuration(); }
...
void readFile(vector<Job>& newJobVector) {
  ...
  if(inFile.is_open()) {
    ...
    newJob1.setArrival(arr_time);
    newJob1.setRuntime(run_time);
    ...
    while (inFile.good() && !inFile.eof()) {
      ...
      newJob.setArrival(arr_time);
      newJob.setRuntime(run_time);
...
void printFileVector (const vector<Job>& myVector) {
  ...
  for(unsigned int i = 0; i < size; i++){
    cout << myVector[i].getJobName() << "\t\t" << myVector[i].getArrivalTime() << "\t\t" << myVector[i].getJobSuration() << endl;
  }
}

void printOutput(const vector<Job>& jobs) {
  ...
  for (int j = 1; j <= total_dur; j++){
    if(j == jobs[loopme].getArrivalTime()) {
    ...
    if(sjf_count < sjf_job[sjf_index].getJobSuration()){
